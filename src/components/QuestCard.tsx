...
const QuestCard: React.FC<QuestCardProps> = ({
  ...
  const getCardBaseStyles = () => {
    if (isTracked && !isCompleted) {
      return darkMode ? 'bg-white/5 border-white/5 opacity-70' : 'bg-slate-50 border-slate-100 opacity-70';
    }
  ...
  return (
    <div
      className={`relative p-5 rounded-[30px] border-2 transition-all duration-300 ${getCardBaseStyles()} ${!effectiveIsGreyed && !isCompleted && !isTracked ? 'shadow-sm hover:shadow-md cursor-pointer' : 'cursor-default'}`}
      onClick={(e) => {
      ...
      <div className="flex justify-between items-start mb-3">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            {timeDisplay && (
              <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full flex items-center gap-1 ${timeDisplay.status === 'now'
            ...
            {isTracked && !isCompleted && (
              <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full flex items-center gap-1 bg-slate-200 text-slate-500 dark:bg-white/10 dark:text-slate-400">
